...
export default function Blogs() {
  ...
  const LIMIT = 7; // Same pattern as Projects: 1 featured + up to 6 grid
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${import.meta.env.VITE_BACKENDLINK}/blogs?page=${currentPage}&limit=${LIMIT}`
      )
      .then((res) => {
        console.log("BLOG API RESPONSE:", res.data);
        console.log("BLOGS RECEIVED:", res.data.blogs);
        console.log("NUMBER OF BLOGS:", res.data.blogs?.length);
  ...
  if (blogs.length === 0) {
    return (
      <div className="max-w-5xl mx-auto px-4 md:px-6 mt-8 md:mt-12 mb-24 font-inter">
        <h1 className="font-unbounded text-3xl md:text-5xl font-bold mb-2">
          &lt;Blogs /&gt;
        </h1>
        ...
        <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed border-slate-950 rounded-2xl bg-white dark:bg-slate-900 shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]">
          ...
          <p className="text-xs text-slate-500 max-w-sm">
            Check back soon for new tutorials, thoughts, and technical
            deep-dives!
          </p>
        </div>
      </div>
  ...
  // IMPORTANT:
  // Keep ALL blogs in the grid.
  // This means the featured blog also appears in the grid.
  const gridBlogs = searchQuery ? filteredBlogs : blogs;

  // Global offset for numbering
  const gridOffset =
    currentPage === 1
      ? 1
      : LIMIT + (currentPage - 2) * LIMIT + 1;
